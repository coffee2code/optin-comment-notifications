...
This plugin adds a checkbox to the profile page for users that allows them to opt into receiving a notification email whenever a comment is made to the site.
...
By default, all users of the site have the ability to subscribe to notifications about comments. A filter is provided to facilitate use of code to customize the feature's available.

Note: a "user" is a person with an actual login account for the site. The plugin does not facilitate permitting visitors who do not have an account on the site to be able to subscribe to comments.
...
function restrict_optin_comment_notifications( $default, $caps ) {
	...
	return in_array( $role, array( 'administrator', 'editor' ) );
}
